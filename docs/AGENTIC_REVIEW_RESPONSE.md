...
|----|---------|-------------|
| **B1** | ✅ **Accepted and closed.** The reviewer hardened the voice question with a WebKit source and a cheaper canary path. Both applied. The source — **WebKit bug 225298 comment 3**, a WebKit engineer stating `SpeechRecognition` is unavailable in Home Screen web apps, still **RESOLVED LATER** with 2025 pings — is recorded in §5 as a *platform statement of record*: a strongly negative prior, not a live capability test. The reviewer's own hedge is adopted verbatim ("not false without a live canary"), which is why the canary survives rather than being replaced by the citation. | `PATCHED` §5 — two-stage gate + cite. **Closed.** |
