...
1. **Consumer product, built to a consumer bar** *(amended 2026-07-26)*. GymOps was personal-first; it is now being built for strangers. Anything that only works for the developer is out of scope. Rationale unchanged: "works for me" is a finish line already crossed. Prioritisation rule *(amended)*: prefer work that moves a stranger from install to logged session. Work that also pays back the current user still wins ties — it is no longer the tiebreak itself.
2. **No monetization planned; option kept open via architecture, not pricing** *(amended 2026-07-26)*. The optionality-preserving investments are stable exercise IDs and real backup/restore. **BYOK is retired** (see [AGENTIC_VISION.md](AGENTIC_VISION.md) §4.1): AI is delivered by a shared key behind auth + budgets, or not at all — and logging is never coupled to AI, free or paid. Stripe/tiers/entitlements remain deferrable, but the Phase 7 server-side-AI trigger below is now **pulled**, not hypothetical.
...
*Added 2026-07-22 after Phase 5 retrospective. Core closeout was **5.7 + 5.8 + 5.9** — **all three shipped** (5.8 July 22 v6.1/gymops-v89; 5.7 + this 5.9 docs update July 23 v6.2/gymops-v90).*

*✅ **The backlog is now empty too, as of July 29, 2026** — 5.10 shipped (`v6.15`/`gymops-v103`), 5.12 closed as a verified no-op, 5.11 refused. **Nothing Phase 5 remains open.** Worth recording that the three resolved as one decision rather than three tasks: 5.12 is a no-op *because* 5.11 was refused, and had 5.11 been built, 5.12's fix would have belonged inside it.*
...
| **5.10** | Empty-day guard in plan editor | Should | XS | ✅ **SHIPPED** July 29, 2026 (app `v6.15`, SW `gymops-v103`). Option taken: **block Save and name the day** — `savePlan` no longer `.filter`s empty days away. Message: `"Day 2" has no exercises. Add one, or remove the day.` (plural variant lists every empty day). Unnamed days use the positional label `_dayLabel` — the same label `savePlan` persists, so the message names the day the user would have got. Ordering: a missing plan name, and a plan with *nothing* in it, both keep the generic message — naming "Day 1" at someone who hasn't added anything describes the editor's internals. `#plan-save-error` gained `role="alert"`: the error is now **blocking**, and a blocking error that doesn't announce is a dead end for a screen-reader user (matches `#input-error`, the other blocking validation error). 7 unit tests + 1 a11y guard, both verified to fail on reintroduction; 18-check CDP run at a real 375px. |
| **5.11** | Rename from active-session exercise (second entry) | Optional | S | ⛔ **REFUSED** July 29, 2026 (owner decision). Rationale: `.exercise-row` already packs name + Plan chip + day chip + Change at 375px, and the active screen's standing constraint is that everything stays above the fold — a rename control is a **non-logging affordance competing for the app's most valuable pixels**, against a north star of "the fastest logger". The papercut it addresses (a mis-typed custom name mid-session) is already survivable: finish, then History → Rename corrects **every** set retroactively, because history follows the denormalised `exercise` column. Joins the refused list alongside social, catalogue browsing and ML coaching. **If ever reopened**, the cache fix is not optional — see the invariant recorded at the rename block in `js/history.js`. |
| **5.12** | Post-rename UI refresh sanity | Should | XS | ✅ **CLOSED — verified no-op** July 29, 2026. No code change, which was the correct outcome. Audit confirmed: `_recencyRanks` rebuilds on picker open, history/plans/idle all re-query, and the DB layer's rename guarantees are already test-covered (`tests/exercises.test.js`). The single in-memory copy of an exercise name is `state.exercise`, and it **cannot go stale**: the exercise-history screen is reachable only from idle, so no live or just-finished session can be on screen while the rename modal is open (active is a Back-root; completed exits one-way to idle; `resumeLastWorkout`'s finish-time copy is read only from that screen). That is a **reachability** argument, not a property of the cache — so it is recorded as a comment at the rename block in `js/history.js` rather than left implicit, because 5.11 is exactly what would break it. |
...
✅ **All four conditions met as of July 23, 2026** (5.7 + 5.8 shipped, 153/153 tests green). **Phase 5 is fully complete.** Its closeout backlog was cleared separately on **July 29, 2026** (5.10 shipped, 5.12 verified, 5.11 refused) — those three were never part of this completion bar, so clearing them changes nothing about the phase verdict; it just means there is no Phase 5 work left to carry forward.
