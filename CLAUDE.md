...
## Phase 5 Status
✅ **CORE COMPLETE** (July 22, 2026, SW cache: `gymops-v87`, app: `v5.10`) — 5.1–5.6 shipped: exercise IDs, multi-day programs, feedback batch, start chooser, quick-log hero, IndexedDB storage, adherence rework. ⚠️ **Closeout open** — rename UI (5.7) still needed for full Phase 5; **5.8 shipped**. Full list + working order in `docs/PHASE4_CONSUMER_PLAN.md` § Phase 5 closeout backlog. Remaining core closeout = **5.7 + 5.9**; should-do 5.10/5.12; optional 5.11. Aliases stay with **6.2**, not Phase 5.
- [ ] **5.7 Exercise rename UI** — BACKLOG. `dbRenameExercise` exists (5.1, throws on clash/empty — UI must catch → toast); History → exercise detail surface still needed. **Blocker decision:** renaming a catalogue exercise re-seeds the old name as a blank row on next boot (`_syncExercises` keys on name) — pick a rule (recommend: set `is_custom=1` on rename). Detail in `docs/PHASE4_CONSUMER_PLAN.md` § closeout.
