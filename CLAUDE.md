...
- [ ] **5.2.x User-feedback batch** — IN PROGRESS. Five small fixes from real-use feedback (July 15 + 18, 2026; quick-log feedback reported twice):
  ...
  5. **lbs↔kg inline converter**: helper text under the weight input showing the converted value as you type (gym plates in kg, logging in lbs). The cheap 80% of the parked mid-session-unit-switch debt; data layer untouched.
- [ ] **5.6 Quick-log prominence design pass** — queued AFTER 5.3 (deliberate, touches the money-path hierarchy): when a plan is linked and a quick-log reference exists, should "Same as last time" outrank the manual inputs? Feedback says yes ("button is awesome", asked for it to be bigger); needs its own design session, not a ride-along in a fix batch.
