...
- `js/plans.js` — Plan banners (`computePlanExpiryBanner`/`computePlanNudge`/`computePlanNudgeBanner`), plans screen, plan editor, `renderPlanAdherence`. **`_openEditor(draft)` is the single way the editor opens** — new/edit/any future proposer are all drafts through it; nothing else may write those nine fields, or a stale value from the last plan survives into the next one (`tests/plan-editor.test.js`).
...
3. Update the service worker cache version in `sw.js` if any cached files changed. Current version: `gymops-v102`. New JS files must be added to the `ASSETS` list in `sw.js` or offline mode breaks.
...
- [ ] **5.10 Empty-day guard in plan editor** — BACKLOG (should). Reframe (audit): `savePlan` *already* drops empty days silently (`.filter(d => d.exercises.length)`) — 5.10 = **surface** the rule (message / block Save) instead of silently deleting a day the user added.
- [ ] **5.11 Rename from active session** — BACKLOG (optional second entry for 5.7). Must also update in-memory `state.exercise` or the active screen shows the stale name (the one real post-rename cache).
- [ ] **5.12 Post-rename UI refresh** — BACKLOG (should). De-risked: `_recencyRanks` rebuilds on picker open, history/plans re-query DB — History-triggered rename is already correct next visit. Only live cache is `state.exercise` (via 5.11). Verify, don't pre-engineer.
