...
- `js/plan-import.js` — AI routine import (Plans → "Import with AI", v6.4, unmerged branch): `openPlanImport`/`importPlanFromPrompt`/`closePlanImport` UI + `_toDraft`/`_mapExercise` (maps model JSON onto the catalogue, clamps/caps untrusted output), then `openPlanFromDraft` (plans.js) opens the editor for review. Calls `/api/generate-plan`. Never auto-saves.
...
- `api/generate-plan.js` — Vercel serverless function (v6.4, unmerged branch). Same Anthropic proxy shape as ai-summary; accepts `{ prompt, apiKey, exercises }`, `system`-prompts a JSON-only structured plan (days→exercises→sets/reps), returns `{ plan }` or `{ error }`. Consumed by `js/plan-import.js`. Env-var key fallback kept unset in prod (BYOK).
...
## Phase 6 Status
🚧 **IN PROGRESS** — working order: 6.4 → 6.6+6.8 → 6.1 → 6.2 → 6.3 → 6.5 → 6.7 → 6.9. Interrupted 2026-07-25 by a **v6.3 user-feedback batch** landed ahead of the queue.
- [ ] **v6.4 AI routine importer** — BUILT on this branch (`feature/ai-routine-import`), **NOT merged — pending strategic review**. Open question: is this an *importer* (transcribe a named routine the user follows — on-strategy, pairs with 6.2) or a *generator* (invent a program from a goal — the "not a coaching app" line)? Plus a weak BYOK gate for a consumer feature and hallucination/trust risk on "celebrity routines". Feedback item: "add other people's routines via AI, e.g. Matt Damon's Odyssey routine." Plans screen **✨ Import with AI** (`#btn-import-plan`) → `#plan-import-modal` → new serverless **`api/generate-plan.js`** (mirrors ai-summary.js: `claude-fable-5` + opus fallback, BYOK, env var unset; `system` passes the 114-name catalogue, demands JSON-only; `_extractJSON` tolerant) → **`js/plan-import.js`** maps names onto the catalogue (`_norm` case/punct-insensitive; unknown → custom), clamps/caps untrusted output (days≤7, exercises≤12), then `openPlanFromDraft` (plans.js) populates the existing editor titled "Review Plan" — proposal, never auto-writes. 7 tests in tests/plan-import.test.js (162 total on this branch). Browser-verified UI + mapping→editor; live /api call untested (needs Vercel runtime + real key).
