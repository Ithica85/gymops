...
3. Update the service worker cache version in `sw.js` if any cached files changed. Current version: `gymops-v94`. New JS files must be added to the `ASSETS` list in `sw.js` or offline mode breaks.
...
**Agentic AI ideas (idea-stage, discussed 2026-07-23)** — future-phase candidates, not scheduled. Each routes through existing Layer 1 functions or read-only queries rather than adding an autonomous write path:
- **Voice logging (push-to-talk quick-log)** — Web Speech API prefills the weight/reps inputs from spoken input ("bench 100 for 8"); user confirm-taps before it writes, never silent auto-log. Calls the existing `logSet`/`quickLogSet` — no new schema, no runtime deps (Web Speech API is browser-native). Keeps Layer 1's "nothing sits between a tap and a logged set" rule by never routing an unconfirmed LLM parse into the critical path.
- **Natural-language plan builder** — LLM drafts a structured plan (days/exercises/targets) from a one-line goal ("4-day upper/lower, focus bench and squat"); user reviews/edits before it ever calls `dbSavePlanExercises` — a proposal, not an autonomous write.
- **Strong/Hevy import mapper** — folds into 6.2's CSV import: LLM maps arbitrary export columns/exercise names onto the schema instead of a per-app regex parser, flagging ambiguous rows for confirmation rather than guessing silently.
- **Read-only history Q&A** — chat-style queries ("how many times did I bench over 100kg this month?") answered by giving the LLM a whitelist of existing `dbGet*` query functions as tools — no raw SQL, no writes. Same trust profile as the existing post-workout AI summary, just interactive.
- **Exercise alias suggestions** — ties into the 6.2 aliases item: suggest merging a custom "Other" name into an existing catalogue entry instead of fragmenting history. Advisory only, never auto-merges.
- **Explicitly rejected for now: plateau/deload detection agent** — conflicts with the "not a coaching app" positioning and the deliberate choice (Phase 2 tech debt) to keep the F-03/F-06 signal engines deterministic and rule-based rather than LLM-judged. Revisit only if that positioning itself changes.