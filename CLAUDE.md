...
3. Update the service worker cache version in `sw.js` if any cached files changed. Current version: `gymops-v91`. New JS files must be added to the `ASSETS` list in `sw.js` or offline mode breaks.
...
## Phase 6 Status
🚧 **IN PROGRESS** — working order: 6.4 → 6.6+6.8 → 6.1 → 6.2 → 6.3 → 6.5 → 6.7 → 6.9. Interrupted 2026-07-25 by a **v6.3 user-feedback batch** landed ahead of the queue. (An **AI routine importer** was prototyped alongside it but is parked on branch `feature/ai-routine-import` pending a strategic review — importer-vs-generator positioning vs the "not a coaching app" stance, BYOK gate, hallucination/trust; not on `main`.)
