...
- `scripts/` — Build-time generators, **not app assets and never SW-cached**: `render-icons.sh` (SVG → PNG icons) and `shoot-screenshots.mjs` (CDP screenshot capture). Nothing here ships to the browser.
- `icons/` — PWA install assets (6.5). `icon-source.svg` / `icon-maskable-source.svg` are the **checked-in sources**; the PNGs are rendered from them by headless Chrome (see the 6.5 entry for the regeneration recipe) — edit the SVG and re-render, never the PNG. `screenshots/` holds the three narrow-form-factor manifest screenshots.
...
3. Update the service worker cache version in `sw.js` if any cached files changed. Current version: `gymops-v96`. New JS files must be added to the `ASSETS` list in `sw.js` or offline mode breaks.
...
## Phase 6 Status
🚧 **IN PROGRESS** — working order: 6.4 → 6.6+6.8 → 6.1 → 6.2 (all shipped) → 6.3 → 6.5 → 6.7 → 6.9. Interrupted 2026-07-25 by a **v6.3 user-feedback batch** landed ahead of the queue, then 2026-07-26 by **v6.4 picker specificity fixes** (bugs surfaced by the Claude Design bundle, not a planned item). **Next: 6.3 accessibility** — three AA contrast failures are already measured and recorded under "Design system mirror" below.
