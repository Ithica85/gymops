...
3. Update the service worker cache version in `sw.js` if any cached files changed. Current version: `gymops-v93`. New JS files must be added to the `ASSETS` list in `sw.js` or offline mode breaks.
...
## Phase 6 Status
🚧 **IN PROGRESS** — working order: 6.4 → 6.6+6.8 → 6.1 → 6.2 → 6.3 → 6.5 → 6.7 → 6.9. Interrupted 2026-07-25 by a **v6.3 user-feedback batch** landed ahead of the queue, then 2026-07-26 by **v6.4 picker specificity fixes** (bugs surfaced by the Claude Design bundle, not a planned item).
